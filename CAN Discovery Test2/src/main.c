...
/*
        uint8_t i = 0;
          while((CAN_TransmitStatus(CANx, mailbox)  !=  CANTXOK) && /*(i  !=  0xFFF)(CAN_TransmitStatus(CANx, mailbox)  != CANTXFAILED))
          {
            i++;
          }
*/
...
void CAN_Config(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin = CAN_RX_PIN | CAN_TX_PIN | CAN2_RX_PIN | CAN2_TX_PIN;
  ...
  CAN_Init(CAN2x, &CAN_InitStructure);
  /* CAN filter init */
#ifdef  USE_CAN1
  ...
  CAN_ITConfig(CAN2x, CAN_IT_FMP0, ENABLE);
  CAN_ITConfig(CANx, CAN_IT_FMP0, ENABLE);
  //CAN_ITConfig(CANx,CAN1_RX0_IRQHandler, ENABLE);
}
